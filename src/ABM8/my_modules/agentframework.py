...
class Agent:
    def __init__(self, agents, i, environment, n_rows, n_cols):
        ...
        """
        # Store the agents, agent index, environment, and dimensions of the environment
        self.agents = agents
        self.i = i
        self.environment = environment
        # Calculate the third of the number of columns and rows and choose a random location within that range
        tnc = int(n_cols / 3)
        self.x = random.randint(tnc - 1, (2 * tnc) - 1)
        tnr = int(n_rows / 3)
        self.y = random.randint(tnr - 1, (2 * tnr) - 1)
        # Set the initial store and store shares to 0
        self.store = 0
        self.store_shares = 0
